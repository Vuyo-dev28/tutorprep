-- Fix past_papers table and ensure bucket exists
...
begin
  ...
  -- Ensure subject column exists and is populated
  if not exists (
    select 1 from information_schema.columns 
    where table_schema = 'public' 
    and table_name = 'past_papers' 
  ...
  -- Populate subject from subject_name if subject is null
  if exists (
    select 1 from information_schema.columns 
    where table_schema = 'public' 
    and table_name = 'past_papers' 
    ...
    where (subject is null or subject = '') and subject_name is not null;
  end if;

  -- Add grade column if missing (for single grade)
  if not exists (
    select 1 from information_schema.columns 
    where table_schema = 'public' 
    and table_name = 'past_papers' 
  ...
  -- Populate grade from grade_from if grade is null
  if exists (
    select 1 from information_schema.columns 
    where table_schema = 'public' 
    and table_name = 'past_papers' 
    ...
    update public.past_papers 
    set grade = grade_from 
    where grade is null and grade_from is not null;
  end if;
  ...
  -- Populate term from past_paper_terms if term_id exists
  if exists (
    select 1 from information_schema.columns 
    where table_schema = 'public' 
    and table_name = 'past_papers' 
    ...
    where pp.term_id = ppt.id and (pp.term is null or pp.term = 0);
  end if;
end $$;
